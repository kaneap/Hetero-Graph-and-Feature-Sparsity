...
def _add_path_shortcut(data, device, path, new_edge_type):
    adjacency_tensors = []
    for edge in path:
        sizes = data[edge[0]].x.shape[0], data[edge[2]].x.shape[0]
        adjacency_tensor = to_torch_sparse_tensor(data[edge].edge_index, size=sizes).detach().clone()
        adjacency_tensors.append(adjacency_tensor)

    multiplied_tensor = torch.eye(adjacency_tensors[0].shape[0])
    ...
    #author_to_conference = torch.mm(author_to_paper, paper_to_conference).to_dense()
